...
def test_graph_creator_three_routers_without_cycle_output_expected_graph():
    ...
    expected_graph = frozenset(
        {
            RouterNode(router=first_router, adjacents=[second_router_node]),
            RouterNode(router=second_router, adjacents=[first_router_node, third_router_node]),
            RouterNode(router=third_router, adjacents=[second_router_node]),
        }
    )
    actual_graph = graph_creator.get_graph()
...
def test_graph_creator_three_routers_cycled_output_expected_graph():
    ...
    expected_graph = frozenset(
        {
            RouterNode(router=first_router, adjacents=[second_router_node, third_router_node]),
            RouterNode(router=second_router, adjacents=[first_router_node, third_router_node]),
            RouterNode(router=third_router, adjacents=[first_router_node, second_router_node]),
        }
    )
    actual_graph = graph_creator.get_graph()
