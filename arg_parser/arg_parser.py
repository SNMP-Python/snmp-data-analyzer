...
class ArgParser:
    ...
    @staticmethod
    def _parse_args(args: Optional[List[str]]) -> Tuple[Optional[str], Optional[str], Optional[str], bool]:
        """
        Returns a tuple which includes:
        ip_address or None, output file or None, community or None, add routes boolean
        """
    ...
    def get_objects_from_args(args_to_parse: Optional[List[str]] = None) -> Tuple[RouterSearcher, Logger, Printer]:
        ip_address, output_file, community, add_routes = ArgParser._parse_args(args_to_parse)
