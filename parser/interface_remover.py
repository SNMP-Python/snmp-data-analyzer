...
class InterfaceRemover:
    ...
    def remove_down_and_loopback(routers: List[Router]):
        InterfaceRemover._remove_interfaces(
            routers=routers, filter_strategy=lambda interface: interface.status != InterfaceStatus.DOWN
        )
        InterfaceRemover._remove_interfaces(
            routers=routers, filter_strategy=lambda interface: interface.type_interface != InterfaceType.LOOPBACK
        )
    ...
    def _remove_interfaces(filter_strategy: Callable[[Interface], bool], routers):
        for router in routers:
            new_interfaces: List[Interface] = [
                interface for interface in router.interfaces if filter_strategy(interface)
            ]
